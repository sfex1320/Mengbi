...
import { runWithUpstream, comfyInputSlots, cancelComfy } from '@/lib/smartCanvasRunner';
import { localPathToImageUrl } from '@/lib/imageUrl';
import type { ComfyNodeData } from '@shared/smartCanvas';
import { NodeShell } from './NodeShell';
import { MeasuredThumb } from '../MeasuredThumb';
import { fmtDur, areaMenu, copyText, makePromptNodeFrom } from '../nodeArea';
...
/** ComfyUI 节点：绑定工作流模板，运行后取回输出图。详细控件在右侧检查器编辑。 */
  ...
  return (
    <>
      ...
      <div className="mb-sc-work-model">
        {d.workflowId ? `输入：${slots.text.length} 词 · ${slots.image.length} 图` : '右侧检查器里选模板'}
      </div>
      <div className="mb-sc-work-runrow nodrag">
        <button
          className="mb-btn mb-btn-sm mb-btn-primary"
          disabled={running || !d.workflowId}
          onClick={() => void runWithUpstream(id)}
        >
          {running ? '运行中…' : '运行'}
        </button>
        {running && (
      ...
      {d.result?.images && d.result.images.length > 0 && (
        <div className="mb-sc-work-thumbs nodrag">
          {d.result.images.slice(0, 4).map((p, i) => (
            <MeasuredThumb key={i} src={imgUrl(p)} alt={`输出 ${i + 1}`} title="结果 · 角标=实际分辨率" onClick={() => openPreview(imgUrl(p))} />
          ))}
        </div>
      )}
      {texts.length > 0 && (
